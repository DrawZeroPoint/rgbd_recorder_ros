...
pcl::PointXYZ GetFeatures::projectDepthTo3D(
    const cv::Mat & depthImage,
    float x, float y,
    float cx, float cy,
    float fx, float fy,
...
PointCloud::Ptr GetFeatures::cloudFromDepthRGB(const cv::Mat & imageRgb, const cv::Mat & imageDepth,
...
void GetFeatures::getFeatures(cv::Mat color, cv::Mat depth, float fx, float fy, float cx, float cy,
                              float maxDepth, float minDepth, PointCloud::Ptr &cloudSource)
{
