...
RefineDepth::RefineDepth(string proto_folder, int gpu_id)
{
#ifdef CPU_ONLY
...
#endif
  string proto_file;
  proto_file = proto_folder + "/refine_depth.prototxt";
...
void RefineDepth::refineDepth(Mat rgb_in, Mat depth_in, Mat &depth_out)
{
  ...
  for (size_t r = 0; r < h_; ++r) {
    for (size_t c = 0; c < w_; ++c) {
      float depth_val = depth_in.at<ushort>(r, c);
      if (depth_val == 0) {
  ...
  depth_out = Mat(h_, w_, CV_16UC1, Scalar(0));

  Mat depth_filled(h_, w_, CV_16UC1, Scalar(0));
  forward(depth_values_in, features_in, features_out, depth_filled);

  for (size_t r = 0; r < h_; ++r) {
    for (size_t c = 0; c < w_; ++c) {
      ushort depth_val = depth_in.at<ushort>(r, c);
      if (depth_val == 0) {
        depth_out.at<ushort>(r, c) = depth_filled.at<ushort>(r, c);
      }
      else {
        depth_out.at<ushort>(r, c) = depth_val;
      }
    }
  }
...
void RefineDepth::getOutput(Mat &depth_out)
{
  ...
  for (int i = 0; i < temp_depth.size(); ++i) {
    int r = i / w_;
    int c = i % w_;
    depth_out.at<ushort>(r, c) = ushort(temp_depth[i]);
  }
}
